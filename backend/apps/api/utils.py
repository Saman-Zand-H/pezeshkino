...
def get_available_times(office_id: int, date_obj: date) -> QuerySet[AvailabilityTime]:
    if not (
        AvailabilityDay.objects.filter(
            Q(day_of_week=date_obj.isoweekday()) & Q(office__id=office_id)
        ).exists()
    ):
        return
    taken_appointments = Appointment.objects.filter(
        Q(office__id=office_id) & Q(datetime__date=date_obj)
    )
    free_times = (
        AvailabilityTime.objects.filter(
            Q(day__day_of_week=date_obj.isoweekday()) & Q(day__office__id=office_id)
        )
...
def get_available_times_in_range(
    office_id, timestamp=timezone.now().timestamp(), date_range=7
    ...
    for i in range(date_range):
        ...
        if available_time and available_time.exists():
            results.append({"obj": available_time, "date": date_delta})
    return results
