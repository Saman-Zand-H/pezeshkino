...
from .utils import (
    get_earliest_available_appointment,
...
from doctors.models import AvailabilityTime, AvailabilityDay, DoctorOffice, Doctor
...
from .serializers.payments import TrackIdSerializer, TransactionSerializer
...
from doctors.queries import (
    get_revenues_for_last_week,
...
class ListSpecialtiesView(APIView):
    def get(self, request, format=None):
        specs = [{"display_name": i.label, "code_name": i.value} for i in Specialties]
        return Response(data=specs, status=status.HTTP_200_OK)
...
class InitiateAppointmentView(APIView):
    ...
    def post(self, request, format=None):
        ...
        if not check_availability(office_id, datetime_obj):
            return Response(
                {"message": "this time is not available for appointment."},
                status=status.HTTP_406_NOT_ACCEPTABLE,
            )

        availability_day_obj = AvailabilityDay.objects.get(
            office=office_id, day_of_week=datetime_obj.isoweekday()
        )
        availability_time_obj = AvailabilityTime.objects.get(
            time=datetime_obj.time(), day=availability_day_obj
        )
        ...
        try:
            ...
            return Response(
                {"message": "there's a problem with the gateway."},
...
class ValidateTransactionView(APIView):
    ...
    def _handle_bank_request(self, trackId, transaction_qs):
        ...
        if result_status == 100:
            ...
            return Response(
                TransactionSerializer(transaction_qs.last()).data,
                status=status.HTTP_200_OK,
            )
...
class ListByTransactionsView(APIView):
    def get(self, request, format=None):
        return Response(
            TransactionSerializer(request.user.by_transactions.all(), many=True)
        )
...
class ListForTransactionsView(APIView):
    def get(self, request, format=None):
        return Response(
            TransactionSerializer(request.user.for_transactions.all(), many=True)
        )
...
class ReviewView(APIView):
    ...
    def get(self, request, format=None):
        ...
        if not doctor_qs.exists():
            return Response(
                {"doctor_username": "no doctor with this username exists."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        return Response(
            ReviewSerializer(reviews, many=True).data, status=status.HTTP_200_OK
        )
...
class RegisterDoctorView(APIView):
    def post(self, request, fromat=None):
        ...
        try:
            ...
            Doctor.objects.create(user=user, specialty=specialty, upin=upin)
            response_data = {}
            ...
            logger.critical(e)
            return Response(
                {"message": "something went wrong. please reach us."},
...
class DashboardHomeDataView(APIView):
    ...
    def get(self, request, format=None):
        ...
        data = {
            "number_of_patients": get_total_number_of_patients(doctor_id),
            "revenue_for_week": get_revenues_for_last_week(doctor_id, timestamp),
            "number_of_appointments": get_number_of_appointments_for_day(
                doctor_id, timestamp
            ),
            "number_of_offices": doctor.doctor_offices.count(),
            "comments": ReviewSerializer(
                doctor.doctor_reviews.all()[:5], many=True
            ).data,
        }
